...
- Produces:
  ...
  - `sync_articulos(index, docs, delete_id_normas) -> list` — returns enqueued tasks
  - `wait_for_tasks(client, tasks) -> None` — raises if any Meilisearch task failed
  - `rank_tipo(tipo: str) -> int`
  - `document_id(id_norma, slug, content_sha256, desde_ts) -> str` — Meilisearch-legal, span-scoped
