...
export function TabBar() {
  ...
  const active: Tab | null =
    open.find(t => t.tipo === params.tipo && t.numero === params.numero && t.date === params.fecha) ?? null
  ...
  const onClose = (t: Tab, e: React.MouseEvent) => {
    ...
    if (next.length === 0) {
      ...
      router.push(canonicalHref(fallback, fallback.date))
    }
  }
        ...
        {open.map(t => {
          ...
          return (
            <button
              ...
              onClick={() => router.push(canonicalHref(t, t.date))}
